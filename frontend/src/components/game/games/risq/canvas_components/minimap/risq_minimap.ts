...
import { drawHexagon, drawRect } from '../../../../util/canvas_util';
...
export class RisqMinimap implements CanvasComponent {
  ...
  private localToCoordinate(local: Point2D): Point2D {
    const board_size = this.risq.getGame()?.board_size ?? 0;
    const cy = (local.y - 0.25 * this.hex_r) / (1.5 * this.hex_r) - board_size - 0.5;
    return {
      x: local.x / (1.732 * this.hex_r) - 0.5 * cy - board_size - 0.5,
      y: cy,
    };
  }

  private coordinateToLocal(coordinate: Point2D): Point2D {
    const board_size = this.risq.getGame()?.board_size ?? 0;
    return {
      ...
      () => {
        ...
        for (const row of game.spaces) {
          for (const space of row) {
            const local = this.coordinateToLocal(space.coordinate);
            ...
            if (space.visibility === RisqVisibilityLevel.UNEXPLORED || view_mode === RisqViewMode.OWNERSHIP) {
              ...
              drawHexagon(ctx, local, draw_r);
            } else {
              drawHexImage(ctx, this.risq.getIcon(terrainImage(space.terrain)), local, draw_r);
        ...
        const viewport_min = this.localFromWorld(
          { x: (transform.view.x + visible_x0) / transform.scale, y: transform.view.y / transform.scale },
          transform.scale
        );
        const viewport_max = this.localFromWorld(
          {
            x: (transform.view.x + visible_x1) / transform.scale,
            y: (transform.view.y + this.risq.canvasSize().height) / transform.scale,
          },
          transform.scale
        );
  ...
  private localFromWorld(world: Point2D, scale: number): Point2D {
    const coordinate = this.risq.canvasToCoordinate(world, scale, this.risq.getGame()?.board_size ?? 0);
    return this.coordinateToLocal(coordinate);
  }
  ...
  private jumpTo(local: Point2D) {
    const coordinate = this.localToCoordinate({
      x: local.x - this.xi() - RisqMinimap.PADDING,
      y: local.y - this.yi() - RisqMinimap.PADDING,
    });
